...
// simulate fairness: a philosopher allows himself M steps of eating or thinking before blocking
// however, he must eat at least once in those M steps
#define M 2
byte fair[N];
bit eaten[N];

inline reset_fairness() {
  int i = 0;
  do
  :: i < N -> atomic { 
                fair[i] = M;
...
proctype philosopher(int p; bool leftFirst) {
  do
  :: (fair[p]>0 && eaten[p]) || (fair[p]>1 && !eaten[p]) -> think(p) /* philosopher decides to think */
  :: fair[p]>0 -> /* philosopher decides to eat */
